...
@Component
public class CreateCreditCardExecutor {

	private static final Logger logger = LoggerFactory.getLogger(CreateCreditCardExecutor.class);
	...
	@Autowired
	private AbstractActor<CreateCreditCardRQ> createCreditCardActor;
	...
	public void scheduleTaskWithFixedRate() {
		...
		List<CreateCreditCardRQ> listMessageRQ = null;
		try {

			connect();

			FTPFile[] requestList = ftpClient.listFiles(Constant.PATH_CREDIT_CARD_REQUEST);
			String currentFile = null;
			String targetFile = null;
			String renameFile = null;
			for (FTPFile ftpFile : requestList) {
				currentFile = String.format("%s%s", Constant.PATH_CREDIT_CARD_REQUEST, ftpFile.getName());
				targetFile = String.format("%s%s_%s", Constant.PATH_CREDIT_CARD_RESPONSE, ftpFile.getName(),
						dateTimeFormatter.format(LocalDateTime.now()));
				renameFile = String.format("%s%s_%s", Constant.PATH_CREDIT_CARD_PROCESSED, ftpFile.getName(), dateTimeFormatter.format(LocalDateTime.now()));
				...
				if (FTPReply.CANNOT_OPEN_DATA_CONNECTION != ftpClient.getReplyCode()
					...
					if (inputStream != null) {
						listMessageRQ = this.createCreditCardActor.read(inputStream);
						inputStreamRS = this.createCreditCardActor.write(listMessageRQ);
