<xsl:stylesheet version="1.0"
	...
	<xsl:template match="/">
		<xsl:if
			test='contains(/ns0:BusinessEvent/ns0:BusinessEventRequest/ns0:products/ns0:cards/ns0:operation/customer-v1-0:operationType/text(),"add")'>
			<xsl:if
				test='/ns0:BusinessEvent/ns0:BusinessEventRequest/ns0:products/ns0:cards/ns0:account/customer-v1-0:cardType/text()="DEBIT"'>
				<TCP_PLAIN_TEXT>
					<xsl:value-of
						select="concat('01',format-number(2300043430030403, '0000000000000000'),'01')" />
				</TCP_PLAIN_TEXT>
			</xsl:if>
			<xsl:if
				...
				<TCP_PLAIN_TEXT>
					<xsl:value-of
						select="concat('01',format-number(2300043430030403, '0000000000000000'),'02')" />
				</TCP_PLAIN_TEXT>
			</xsl:if>
		</xsl:if>
		<xsl:if
			...
			<xsl:if
				...
				<TCP_PLAIN_TEXT>
					<xsl:value-of
						select="concat('02',format-number(2300043430030403, '0000000000000000'),'01')" />
				</TCP_PLAIN_TEXT>
			</xsl:if>
			<xsl:if
				test='/ns0:BusinessEvent/ns0:BusinessEventRequest/ns0:products/ns0:cards/ns0:account/customer-v1-0:cardType/text()="CREDIT"'>
				<TCP_PLAIN_TEXT>
					<xsl:value-of
						select="concat('02',format-number(2300043430030403, '0000000000000000'),'02')" />
				</TCP_PLAIN_TEXT>
			</xsl:if>
		</xsl:if>
	</xsl:template>
